...
public class Overlay {
    ...
    public void highlightElement(OverlayElement element) {
    ...
    public Overlay(ScathaPro scathaPro) {
        ...
        DynamicOverlayContainer countersContainer =
                new DynamicOverlayContainer(0, 0, 1f, Direction.HORIZONTAL).setMargin(0, 4);
    ...
    public void drawOverlayIfAllowed(MatrixStack matrices) {
        if (isOverlayDrawAllowed()) drawOverlay(matrices);
    }
    ...
    int x = highlightedElement.getAbsoluteX();
    int y = highlightedElement.getAbsoluteY();
    int w = highlightedElement.getScaledWidth();
    int h = highlightedElement.getScaledHeight();
    ...
    public void updatePosition() {
        OverlayPosition pos = scathaPro.getConfig().getEnum(Config.Key.overlayPosition, OverlayPosition.class);
        ...
        switch (pos) {
            ...
            case TOP_CENTER -> { x = (screenW - w) / 2; y = 10; }
            case TOP_RIGHT -> { x = screenW - w - 10; y = 10; }

            case MIDDLE_LEFT -> { x = 10; y = (screenH - h) / 2; }
            case MIDDLE_CENTER -> { x = (screenW - w) / 2; y = (screenH - h) / 2; }
            case MIDDLE_RIGHT -> { x = screenW - w - 10; y = (screenH - h) / 2; }

            case BOTTOM_LEFT -> { x = 10; y = screenH - h - 10; }
            case BOTTOM_CENTER -> { x = (screenW - w) / 2; y = screenH - h - 10; }
            case BOTTOM_RIGHT -> { x = screenW - w - 10; y = screenH - h - 10; }
        }
    ...
    public void updateContentAlignment() {
        contentAlignment = scathaPro.getConfig().getEnum(Config.Key.overlayAlignment, Alignment.class);
        if (contentAlignment == null) contentAlignment = Alignment.LEFT;
    }

    // ============================================================
    // UPDATE METHODS (PART 1)
    // ============================================================
