...
public class Overlay {
    ...
    public Overlay(ScathaPro mod) { 
        this.mod = mod; 
        this.root = new DynamicOverlayContainer(10, 10, 1f);
    }

    public void tick() { 
        // Hvis du har realtime elementer, opdater dem her 
    }
    ...
    public void render(DrawContext ctx, float tickDelta) {
        ...
        // Scale fra config
        float scale = (float) ScathaPro.CONFIG.getDouble("overlayScale");
        root.setScale(scale);
    ...
    private void buildOverlay() {
        ...
        // Eksempel-elementer
        if (ScathaPro.CONFIG.getBoolean("showChain")) {
            root.add(new OverlayText("Chain: 12", 0xFFFFFF, 0, 0, 1f));
        }

        if (ScathaPro.CONFIG.getBoolean("showTimer")) {
            root.add(new OverlayText("Timer: 00:42", 0xFFFFFF, 0, 12, 1f));
        }

        if (ScathaPro.CONFIG.getBoolean("showChance")) {
            root.add(new OverlayText("Chance: 1.23%", 0xFFFFFF, 0, 24, 1f));
        }
    }
    ...
    public static class ToggleableOverlayElement {
        ...
        if (mod.CONFIG.getBoolean("showChain")) {
            root.add(new OverlayText("Chain: " + chain, 0xFFFFFF, 0, 0, 1f));
        }
