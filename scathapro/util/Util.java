...
import net.minecraft.client.renderer.MatrixStack;
...
public class Util
{
    ...
    {
        WorldClient world = MinecraftClient.getInstance().theWorld;
        if (world == null) return -1;
        return (int) Math.floor(world.getWorldTime() / 24000f);
    }
    ...
    {
        ScoreObjective scoreobjective = MinecraftClient.getInstance().theWorld.getScoreboard().getObjectiveInDisplaySlot(0);
        NetHandlerPlayClient handler = MinecraftClient.getInstance().thePlayer.sendQueue;
        return MinecraftClient.getInstance().gameSettings.keyBindPlayerList.isKeyDown() && (!MinecraftClient.getInstance().isIntegratedServerRunning() || handler.getPlayerInfoMap().size() > 1 || scoreobjective != null);
    }
    ...
    public static void startImageRendering()
    {
        ...
        MatrixStack.tryBlendFuncSeparate(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA, 1, 0);
